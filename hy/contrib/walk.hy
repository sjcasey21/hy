...
(defmacro smacrolet [bindings #* body]
  ...
  (setv bindings (dict (by2s bindings))
        body (macroexpand-all body hy-compiler))
...
(defmacro let [bindings #* body]
  ...
  (for [[k v] (by2s bindings)]
    ...
    (if (destructuring? k)
        (do
          ...
          (.append keys (symbolexpand (macroexpand-all k hy-compiler) destructuring-expander))
          (.append values (symbolexpand (macroexpand-all v hy-compiler) expander))
        ...
        (do (.append values (symbolexpand (macroexpand-all v hy-compiler) expander))
  ...
  `(do
     (setv ~g!let {}
           ~@(sum (zip keys values) (,)))
     ~@(symbolexpand (macroexpand-all body hy-compiler)
                     expander)))
