...
class HyASTCompiler(object):
    ...
    def _compile_looper_parts(self, parts):
        return [
            Tag(
                p.tag,
    ...
    def _compile_manual_loop(self, parts, base_case, orelse=None):
        def f(parts):
    ...
    def _is_real_comprehension(self, elt, parts, key=None):
    ...
    def _create_generator_function(self, expr, parts, base_case, node):
        ...
        ret = Result() + asty.FunctionDef(
            ...
            body=self._compile_manual_loop(parts, base_case).stmts,
            decorator_list=[],
        )
    ...
    def _compile_comprehension_generators(self, parts):
        generators = []
    ...
    def compile_for_loop(self, expr, root, parts, final):
        ...
        parts = self._compile_looper_parts(parts)
    ...
    def compile_iterable_comprehension(self, expr, root, parts, final):
        ...
        if self._is_real_comprehension(elt, parts):
            return node_class(
                expr,
                elt=elt.expr,
                generators=self._compile_comprehension_generators(parts),
            )
        else:
            ret = elt
            val = elt.force_expr
    ...
    def compile_dict_comprehension(self, expr, root, parts, final):
        ...
        # Produce a result.
        if self._is_real_comprehension(elt, parts, key):
            ...
            base_case = ret + asty.Expr(elt, value=asty.Yield(elt, value=val))
            return self._create_generator_function(
                expr, parts, base_case, asty.DictComp
            )
    @special(["not", "~"], [FORM])
